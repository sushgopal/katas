...
void run_test_and_assert(char* roman, int expected) {
    int actual = convert_to_arabic(roman);
    ck_assert_int_eq(actual, expected);
}

START_TEST(_I_to_1) {  
    run_test_and_assert("I", 1);
}
END_TEST

START_TEST(_III_to_3) {
    run_test_and_assert("III", 3);
}
END_TEST

START_TEST(_IV_to_4) {
    run_test_and_assert("IV", 4);
}
END_TEST

START_TEST(_V_to_5) {
    run_test_and_assert("V", 5);
}
END_TEST

START_TEST(_X_to_10) {
    run_test_and_assert("X", 10);
}
END_TEST

START_TEST(_XLV_to_45) {
     run_test_and_assert("XLV", 45);
}
END_TEST

START_TEST(_CMXCIX_to_999) {
     run_test_and_assert("CMXCIX", 999);
}
END_TEST
...
int main(void) {
    ...
    Suite *test_suite = make_roman_to_arabic_converter_test_suite();
